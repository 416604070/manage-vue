    ...
    <el-container class="wrapper">
        <el-scrollbar class="container-left" wrap-class="container-left-wrap"
            ...
            <el-menu
                ...
                unique-opened
                router>
                <sidebar-item v-for="item in menus" :key="item.path" :item='item'></sidebar-item>
            </el-menu>
        </el-scrollbar>
        ...
        <el-container class="container-box" v-bind:class="{'slide-hide': isCollapse, 'slide-in-left': menuShow}">
            <el-header class="header">
                ...
                <!--新增界面-->
                <el-dialog title="修改密码" :visible.sync="passwordFormVisible" width="40%" top="15vh">
                    <el-form :model="passwordFormData" :rules="passwordFormDataRules" ref="passwordForm">
                        <el-form-item label="原始密码" prop="old_password">
                            <el-input type="password" v-model="passwordFormData.old_password"
                                      auto-complete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="新密码" prop="new_password">
                            <el-input type="password" v-model="passwordFormData.new_password"
                                      auto-complete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="确认密码" prop="check_new_password">
                            <el-input type="password" v-model="passwordFormData.check_new_password"
                                      auto-complete="off"></el-input>
                        </el-form-item>
                    </el-form>
                    <div slot="footer" class="dialog-footer">
                        <el-button @click.native="passwordFormVisible = !passwordFormVisible">取消</el-button>
                        <el-button type="primary" @click.native="updatePassword('passwordFormData')">提交
                        </el-button>
                    </div>
                </el-dialog>
        ...
        data() {
            let validatePass = (rule, value, callback) => {
                if (value === "") {
                    ...
                    if (value === this.passwordFormData.old_password) {
                        callback("新密码不能与旧密码一致！")
                    }
                    if (!/^(?![a-zA-z]+$)(?!\d+$)(?![!@#$%^&*]+$)(?![a-zA-z\d]+$)(?![a-zA-z!@#$%^&*]+$)(?![\d!@#$%^&*]+$)[a-zA-Z\d!@#$%^&*]{6,16}$/.test(value)) {
                        callback("密码长度6-16位并且同时包含字母数字特殊字符!")
                    }
        ...
        methods: {
            ...
            gotoLogin() {
                ...
                this.$Http.doPostForForm(
                    ...
                    {},
                    function (self, data) {
                        self.$store.commit(self.$Types.RECEIVE_ACCOUNT_INFO, "");
            ...
            updatePassword() {
                this.$refs["passwordForm"].validate(valid => {
                    if (valid) {
                        let params = {
                            oldPassword: this.$PasswordUtil.getPasswordByBase64(this.passwordFormData.old_password, this.$store.getters.username),
                            newPassword: this.$PasswordUtil.getPasswordByBase64(this.passwordFormData.new_password, this.$store.getters.username)
                        };
                        let url = "account/account/password";
                        this.$Http.doPostForForm(
                            this,
                            url,
                            params,
