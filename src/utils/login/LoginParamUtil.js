...
const getLoginParam = function(username, password, key, verificationCode){
    ...
    // 对账号密码进行加密
    let strRandomIV = randomKey(16);
    let iv = cryptojS.enc.Utf8.parse(strRandomIV);

    let md5Password = PasswordUtil.getPasswordByMD5(password, username);
    let strAesEncrypted = cryptojS.AES.encrypt(Config.appKey, md5Password, {
        iv: iv,
